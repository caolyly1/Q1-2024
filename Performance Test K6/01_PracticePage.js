...
export default function () {
  ...
  const response = http.get('https://ultimateqa.com/automation/');

  // This block defines assertions to validate the response
  check(response, {
    'status is 200': (r) => r.status === 200,
    'body contains "Automation Practice"': (r) => r.body.indexOf('Automation Practice') !== -1, // Use indexOf for substring matching
...
export function handleSummary(data) {
  const reportName = `report_PracticePage.html`;  // Set the desired filename here
  return {
